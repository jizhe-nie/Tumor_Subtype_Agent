...
def _build_ae_dims(input_dim: int, latent_dim: int) -> List[int]:
    if input_dim >= 2000:
        return [input_dim, 1024, 256, latent_dim]
    if input_dim >= 500:
        return [input_dim, 256, 64, latent_dim]
    return [input_dim, 64, latent_dim]


@tool
def scdeepcluster_tool(
    data_path: str,
    n_clusters: int,
    latent_dim: int = 10,
    ...
    """
    scDeepCluster (2019)：PyTorch 版简化实现（AE + DEC，用于单细胞表达矩阵）。
    参数：
    - data_path: CSV 文件路径（行=细胞/样本，列=基因）
    - n_clusters: 期望亚型数量
    - latent_dim: 潜在维度
    ...
    model = DeepClusterModel(ae, centers).to(device)
    ...
    for epoch in range(finetune_epochs):
        ...
        for i, (xb,) in enumerate(loader):
            ...
            idx_end = min((i + 1) * batch_size, len(X))
    ...
    output_dir = resolve_output_dir([data_path], "scDeepCluster")
    labels_path = os.path.join(output_dir, f"scdeepcluster_labels_k{n_clusters}.csv")
